...
<p class="page"> --}}
    ...
    @if (count($tasks_Tassativi)>0)
        <div class="row" style="text-align: center">
            <h3 style="color: rgba(170, 0, 0, 0.919);">PRODUZIONI TASSATIVE!</h3>
        </div>
          @foreach ($tasks_Tassativi as $task)
        @php
        ...
        <div class="row element-that-contains-table" style="padding-top: 5pt;">
            <table style="font-size: medium; font-weight:600;">
                ...
                <tr>
                    ...
                    <td style="background-color: rgb({{ $ralRGB }}); opacity:75%;"></td>
                    <td>{{ $task['values']['ibp_ral_guscio'] ?? '' }}</td>
                    @if (!empty($task['values']['ibp_colonna']))<td>{{ $task['values']['ibp_colonna'] ?? '' }}</td>@else<td>{{
                        $task['values']['ibp_braccio'] ?? '' }}</td>@endif
                ...
                <tr>
                    <td>
                        @if (in_array($task['matricole'][0] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][0] ?? '' }}
                            @if (in_array($task['matricole'][0] ?? '', $matricole_completed)) </del>@endif
                    </td>
                    <td>
                        @if (in_array($task['matricole'][4] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][4] ?? '' }}
                ...
                <tr>
                    <td>
                        @if (in_array($task['matricole'][1] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][1] ?? '' }}
                            @if (in_array($task['matricole'][1] ?? '', $matricole_completed)) </del>@endif
                    </td>
                    <td>
                        @if (in_array($task['matricole'][5] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][5] ?? '' }}
                ...
                <tr>
                    <td>
                        @if (in_array($task['matricole'][2] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][2] ?? '' }}
                            @if (in_array($task['matricole'][2] ?? '', $matricole_completed)) </del>@endif
                    </td>
                    <td>
                        @if (in_array($task['matricole'][6] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][6] ?? '' }}
                ...
                <tr>
                    <td>
                        @if (in_array($task['matricole'][3] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][3] ?? '' }}
                            @if (in_array($task['matricole'][3] ?? '', $matricole_completed)) </del>@endif
                    </td>
                    <td>
                        @if (in_array($task['matricole'][7] ?? '', $matricole_completed)) <strong style="color: green">✓
                        </strong> <del>@endif
                            {{ $task['matricole'][7] ?? '' }}
    ...
    @if (count($tasks_Tassativi)>0 && count($tasks)>0)
    <br>    
        <div class="row" style="text-align: center">
            <h3>PRODUZIONI ORDINARIE!</h3>
        </div>
    @endif
