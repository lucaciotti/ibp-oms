    ...
    <div class="row">
        ...
        <table>
            ...
            <tr>
                ...
                @php
                $countPolistirolo = $tasks->where('ibp_imballo_dim', $imb_dim)->where('ibp_imballo_tipo', substr($item, 0,
                strpos($item, ' +')))
                ->filter(function ($task) { return strpos(strtolower($task->ibp_imballo_note), 'polistirolo') !== false ||
                strpos(strtolower($task->ibp_note_imballo2), 'polistirolo') !== false; })->count()/4;
                $countNONPolistirolo = 0;
                @endphp
